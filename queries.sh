...
echo "$($PSQL "SELECT name FROM teams, games WHERE teams.team_id = games.winner_id AND games.year = 2018 AND games.round = 'Final'")"
...
echo "$($PSQL "SELECT year, name FROM teams, games WHERE teams.team_id = games.winner_id AND round = 'Final' ORDER BY year")"
